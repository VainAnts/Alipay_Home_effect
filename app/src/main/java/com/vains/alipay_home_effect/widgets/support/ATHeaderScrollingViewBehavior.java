package com.vains.alipay_home_effect.widgets.support;
...
public abstract class ATHeaderScrollingViewBehavior extends ATViewOffsetBehavior<View> {
    ...
    public ATHeaderScrollingViewBehavior() {
    }

    public ATHeaderScrollingViewBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    protected void layoutChild(final CoordinatorLayout parent, final View child,
        ...
        if (header != null) {
            ...
            final WindowInsetsCompat parentInsets = getParentLastWindowInsets(parent);
    ...
    private WindowInsetsCompat getParentLastWindowInsets(CoordinatorLayout parent) {
        if (parent == null) {
            return null;
        }
        //noinspection TryWithIdenticalCatches
        try {
            Method getLastWindowInsets = parent.getClass().getDeclaredMethod("getLastWindowInsets");
            getLastWindowInsets.setAccessible(true);
            return (WindowInsetsCompat) getLastWindowInsets.invoke(parent);
    ...
    protected final int getOverlapPixelsForOffset(final View header) {
        return mOverlayTop == 0 ? 0 : ATMathUtils.constrain(
